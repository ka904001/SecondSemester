...
void cl_base ::delete_connect(void (*p_signal)(string), cl_base *p_ob_hendler,
...
void cl_base::emit_signal(void (*p_signal)(string), fstream &file,
                          string &s_command) {
  void (*p_hendler)(cl_base * p_ob, fstream &, string);
  if (connects.empty())
    return;
  if (connects.count(p_signal) == 0)
    return;
  ...
  while (it_connects != connects.end()) {
    if ((it_connects->first) == p_signal) {
      ...
      (p_hendler)(it_connects->second->p_cl_base, file, s_command);
    }
    it_connects++;
  }
}
